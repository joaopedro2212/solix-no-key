...
local GameList = {
	...
	["4658598196"] = { id = "52cc23f3d78c16c959b080fca6d9cd54", keyless = true } -- Attack on Titan Revolution
}
		...
		if Result.code == "KEY_VALID" then
			...
			if not (
				GameId == "3808223175" -- Jujutsu Infinite
					...
					or GameId == "4658598196" -- Attack on Titan Revolution
				) and not getgenv().lilix then
				Library:Notification({
					Title = "Solix Hub",
					Description = "This executor is not supported for this game.",
					Color = Color3.fromRGB(math.random(0, 255), math.random(0, 255), math.random(0, 255)),
					Duration = 5
				})
			end
